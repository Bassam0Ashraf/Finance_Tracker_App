 ...
 * File         : Test.js
 * Description  : Automated Selenium test suites covering all
 *                form fields and transaction list behaviour of
 *                the Finance Tracker App (TC-001 – TC-027).
...
// FinanceTrackerPage : Page Object Model — encapsulates all form interactions (fill, select, click).
// FinancialOverview  : Page Object Model — reads the balance, income, and expenses summary cards.
// TransactionList    : Page Object Model — reads individual transaction rows from the list.
// path               : Node.js built-in module for constructing OS-independent file system paths.
// expect / assert    : Chai assertion interfaces used to validate test outcomes.
// By                 : Selenium locator strategy used to find DOM elements (By.id, By.css, etc.).
//
// Note — Mocha test script path pattern used in package.json:
//   "test": "mocha Tests/Selenium/test/**/*.test.js --timeout 60000"
//   **        → Matches any depth of subdirectory (recursive search).
//   *.test.js → Matches only files whose name ends in .test.js.
...
const { TransactionType, Category } = require('./Helper/Utils/Constants');
...
describe('Description Field Test Suite', function () 
{
 ...
 {
     // Runs once before all tests in the Description Field Test Suite.
     // Opens a fresh browser session, navigates to the application, and verifies
     // the page loaded correctly. All test cases in this suite share this session.
     console.log('\n🟢 Step 1: before() hook started — Description Field Test Suite');
     ...
     try {
         ...
         // Verify that the application loaded by confirming the main heading is present.
         const HeaderPage = await waitFor(driver, By.css('.header h1'), TIMEOUT);
         const HeaderPageText = await HeaderPage.getText();
         console.log('🟢 Step 9: Verifying page header...');

         // Assert the header reads "Personal Finance Tracker" to confirm the correct page loaded.
 ...
 *******************************************************************************************************/
 it('TC-001: Empty Description Field', async function ()
 {
    ...
    await page.selectType(TransactionType.INCOME);
    await page.selectCategory(Category.SALARY.value);
...
before(async function () 
{ 
 // Runs once before all tests in the Amount Field Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Amount Field Test Suite');
...
before(async function () 
{ 
 // Runs once before all tests in the Type Field Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Type Field Test Suite');
 ...
 *******************************************************************************************************/
 it('TC-013: Income Transaction Type', async function ()
 {
    ...
    await page.selectCategory(Category.SALARY.value);
     ...
     expect(lastTransaction.type).to.equal(TransactionType.INCOME);
     expect(lastTransaction.categoryValue).to.equal(Category.SALARY.value);
 ...
 *******************************************************************************************************/
 it('TC-014: Expense Transaction Type', async function ()
 {
    ...
    await page.fillDescription('a');
    await page.fillAmount(100);
    await page.selectType(TransactionType.EXPENSE);
    await page.selectCategory(Category.SALARY.value);
     ...
     expect(lastTransaction.description).to.equal('a');
     expect(lastTransaction.amount).to.equal('$ -100');
     expect(lastTransaction.type).to.equal(TransactionType.EXPENSE);
     expect(lastTransaction.categoryValue).to.equal(Category.SALARY.value);
...
before(async function () 
{ 
 // Runs once before all tests in the Category Field Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Category Field Test Suite');
 ...
 *******************************************************************************************************/
 it('TC-015: Category Field Empty (Optional Field)', async function ()
 {
    ...
    await page.selectType(TransactionType.INCOME);  
     ...
     expect(lastTransaction.amount).to.equal('$ 100');
     expect(lastTransaction.type).to.equal(TransactionType.INCOME);
     expect(lastTransaction.categoryValue).to.equal(Category.NO_CATEGORY.display);
 ...
 *******************************************************************************************************/
 it('TC-016: Category with Special Character (Bills & Utilities)', async function ()
 {
    ...
    await page.fillAmount(100);
    await page.selectType(TransactionType.INCOME); 
    await page.selectCategory(Category.BILLS.value);
     ...
     expect(lastTransaction.amount).to.equal('$ 100');
     expect(lastTransaction.type).to.equal(TransactionType.INCOME);
     expect(lastTransaction.categoryDisplay).to.equal(Category.BILLS.display);
     expect(lastTransaction.categoryValue).to.equal(Category.BILLS.value);
...
before(async function () 
{ 
 // Runs once before all tests in the Date Field Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Date Field Test Suite');
 ...
 *******************************************************************************************************/
 it('TC-018: Date Field Uses Default (Today)', async function ()
 {
    ...
    await page.fillAmount(100);
    await page.selectType(TransactionType.EXPENSE); 
    await page.selectCategory(Category.SHOPPING.value);
 ...
 *******************************************************************************************************/
 it('TC-020: Maximum Valid Date (2099-12-31)', async function ()
 {
    ...
    await page.fillAmount(100);
    await page.selectType(TransactionType.EXPENSE); 
    await page.selectCategory(Category.TRANSPORTATION.value);
...
before(async function () 
{ 
 // Runs once before all tests in the Note Field Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Note Field Test Suite');
...
before(async function () 
{ 
 // Runs once before all tests in the Transaction List Test Suite.
 // Opens a fresh browser session, navigates to the application, and verifies
 // the page loaded correctly. All test cases in this suite share this session.
 console.log('\n🟢 Step 1: before() hook started — Transaction List Test Suite');
 ...
 *******************************************************************************************************/
 it('TC-026: Three Consecutive Transactions — Reverse Chronological Order', async function ()
 {
    ...
    await page.fillAmount(100);
    await page.selectType(TransactionType.EXPENSE); 
    await page.selectCategory(Category.ENTERTAINMENT.value);
     ...
     expect(lastTransaction.amount).to.equal('$ -100');
     expect(lastTransaction.type).to.equal(TransactionType.EXPENSE);
     expect(lastTransaction.categoryDisplay).to.equal(Category.ENTERTAINMENT.display);
     expect(lastTransaction.categoryValue).to.equal(Category.ENTERTAINMENT.value);
     ...
     expect(lastTransaction.type).to.equal(TransactionType.INCOME);
     expect(lastTransaction.categoryDisplay).to.equal(Category.SALARY.display);
     expect(lastTransaction.categoryValue).to.equal(Category.SALARY.value);
     ...
     expect(lastTransaction.amount).to.equal('$ 100');
     expect(lastTransaction.type).to.equal(TransactionType.INCOME);
     expect(lastTransaction.categoryDisplay).to.equal(Category.SHOPPING.display);
     expect(lastTransaction.categoryValue).to.equal(Category.SHOPPING.value);
  ...
  *******************************************************************************************************/
 it('TC-027: Delete Last Transaction', async function ()
 {
     console.log('🔎 TC-027: Delete Last Transaction');

     let index = 0;

     // Step 3 & 4: Add a fresh transaction that will be the target for deletion
    await page.fillDescription('delete me');
    await page.fillAmount(100);
    await page.selectType(TransactionType.INCOME);
    await page.selectCategory(Category.HEALTHCARE.value);

     // Record the transaction count before adding the new transaction
     const countBefore = await transcationlist.getTransactionCount();
     
     // Step 5: Click Add Transaction
     await page.addTransaction();

     // Verify the new transaction was added by confirming the count increased by 1
     const countAfterAdd = await transcationlist.getTransactionCount();
     expect(countAfterAdd).to.equal(countBefore + 1);
     console.log('✅ Transaction added successfully — count increased from', countBefore, 'to', countAfterAdd);

     // Step 6: Click the Delete button for the newest transaction (index 0)
     await page.deleteTransaction(index);

     // Step 7: Verify the transaction was removed by confirming the count returned to its original value
     const countAfterDelete = await transcationlist.getTransactionCount();
     expect(countAfterDelete).to.equal(countBefore);
     console.log('✅ Transaction deleted successfully — count returned to', countAfterDelete);

     console.log('✅ TC-027: Delete Last Transaction — assertion passed');
